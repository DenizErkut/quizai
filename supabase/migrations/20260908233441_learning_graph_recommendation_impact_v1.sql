...
CREATE TABLE IF NOT EXISTS public.recommendation_impact_measurements (
  ...
  subject text NOT NULL, topic text NOT NULL, action_type text NOT NULL, engine_version text NOT NULL,
  graph_used boolean NOT NULL DEFAULT false, graph_edge_id uuid, graph_relation_version integer,
  baseline_mastery numeric(5,2), baseline_retention numeric(5,2), post_mastery numeric(5,2),
  mastery_delta numeric(6,2), event_score_pct numeric(5,2),
  event_count integer NOT NULL DEFAULT 0 CHECK (event_count >= 0),
  applied_at timestamptz NOT NULL, evaluated_at timestamptz,
...
CREATE INDEX IF NOT EXISTS recommendation_impact_graph_time_idx ON public.recommendation_impact_measurements (graph_used, applied_at DESC);
CREATE INDEX IF NOT EXISTS recommendation_impact_subject_action_idx ON public.recommendation_impact_measurements (subject, action_type, applied_at DESC);
CREATE INDEX IF NOT EXISTS recommendation_impact_student_time_idx ON public.recommendation_impact_measurements (student_id, applied_at DESC);
...
BEGIN
  ...
  SELECT recommendation_id,student_id,quiz_session_id,subject,topic,action_type,engine_version,
    graph_edge_id IS NOT NULL,graph_edge_id,graph_relation_version,baseline_mastery,baseline_retention,
    post_mastery,CASE WHEN baseline_mastery IS NOT NULL AND post_mastery IS NOT NULL THEN round(post_mastery-baseline_mastery,2) END,
    event_score_pct,event_count,applied_at,CASE WHEN event_count>0 THEN now() END,
    jsonb_build_object('reasonCode',evidence->>'reasonCode','targetTopic',evidence->>'targetTopic'),now()
  FROM resolved
  ON CONFLICT (recommendation_id,quiz_session_id) DO UPDATE SET
    ...
    graph_edge_id=EXCLUDED.graph_edge_id,graph_relation_version=EXCLUDED.graph_relation_version,updated_at=now();
...
CREATE TRIGGER zz_learning_events_measure_recommendation_impact_v1 AFTER INSERT ON public.learning_events
REFERENCING NEW TABLE AS inserted_learning_events FOR EACH STATEMENT
EXECUTE FUNCTION public.measure_recommendation_impact_from_events_v1();
...
CREATE OR REPLACE VIEW public.learning_graph_recommendation_impact_summary WITH (security_invoker=true) AS
SELECT subject,action_type,graph_used,engine_version,measurement_version,count(*)::bigint AS sample_size,
 count(*) FILTER(WHERE evaluated_at IS NOT NULL)::bigint AS evaluated_count,
 round(avg(event_score_pct),2) AS avg_event_score_pct,round(avg(mastery_delta),2) AS avg_mastery_delta,
 round(avg(baseline_mastery),2) AS avg_baseline_mastery,min(applied_at) AS first_applied_at,max(applied_at) AS last_applied_at
