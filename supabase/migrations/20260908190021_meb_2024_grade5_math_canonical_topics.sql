...
SELECT 'topic:meb-2024:ortaokul:5:matematik:' || md5(lower(t.label)),
  'topic', t.label, 'Matematik', '5. sınıf', 'ortaokul', 'meb',
  jsonb_build_object('catalog_version', 'meb-2024', 'verification_status', 'verified',
    'source_reference', t.source_reference, 'unit_label', t.unit_label), true
FROM canonical_topics t
WHERE NOT EXISTS (
  SELECT 1 FROM public.learning_graph_nodes existing
  WHERE existing.node_type = 'topic' AND lower(existing.label) = lower(t.label)
...
ON CONFLICT (node_key) DO UPDATE SET label = EXCLUDED.label, subject = EXCLUDED.subject,
 grade = EXCLUDED.grade, level = EXCLUDED.level, source_type = EXCLUDED.source_type,
...
FROM canonical_topics t
JOIN LATERAL (
  SELECT n.id FROM public.learning_graph_nodes n
  ...
  ORDER BY (n.source_type = 'meb') DESC, n.created_at LIMIT 1
) topic ON true
JOIN public.learning_graph_nodes unit
  ON unit.node_type = 'unit' AND lower(unit.subject) = lower('Matematik')
 AND public.canonical_learning_grade(unit.grade) = public.canonical_learning_grade('5. sınıf')
 AND lower(unit.label) = lower(t.unit_label) AND unit.is_active = true
...
BEGIN
 ...
 AND topic IN ('1. Tema: Sayılar ve Nicelikler (1)','1. Tema: Sayılar ve Nicelikler (2)',
 '2. Tema: İşlemlerle Cebirsel Düşünme','3. Tema: Geometrik Şekiller',
 '4. Tema: Geometrik Nicelikler','5. Tema: İstatistiksel Araştırma Süreci','6. Tema: Veriden Olasılığa');
 IF v_target_count <> 7 THEN RAISE EXCEPTION 'Expected 7 canonical Grade 5 Mathematics publish targets, found %',v_target_count; END IF;
END $$;
