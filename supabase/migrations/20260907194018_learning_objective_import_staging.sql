...
BEGIN
  ...
  UPDATE public.learning_objective_import_items i SET
    ...
    validation_status = CASE WHEN jsonb_array_length(e.errors) = 0 THEN 'valid' ELSE 'invalid' END,
    updated_at = now()
  FROM LATERAL (
    SELECT coalesce(jsonb_agg(message) FILTER (WHERE message IS NOT NULL), '[]'::jsonb) AS errors
    FROM (VALUES
