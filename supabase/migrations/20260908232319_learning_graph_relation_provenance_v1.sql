...
UPDATE public.learning_graph_edges SET valid_from = coalesce(valid_from, created_at) WHERE valid_from IS NULL;
ALTER TABLE public.learning_graph_edges
...
SET search_path = public, pg_temp AS $$
BEGIN
  IF to_jsonb(OLD) <> to_jsonb(NEW) THEN NEW.relation_version := OLD.relation_version + 1; END IF;
  RETURN NEW;
END; $$;
...
CREATE TRIGGER learning_graph_edge_version_v1 BEFORE UPDATE ON public.learning_graph_edges
FOR EACH ROW EXECUTE FUNCTION public.version_learning_graph_edge_v1();
CREATE TRIGGER learning_graph_edge_audit_v1 AFTER INSERT OR UPDATE OR DELETE ON public.learning_graph_edges
