...
export default function DataLifecycleRequests() {
  ...
  return <div className="card" style={{ marginBottom: '1rem' }}><strong style={{ color: 'var(--primary)' }}>🔐 KVKK talepleri</strong>{items.length ? items.map(item => <div key={item.id} style={{ borderTop: '1px solid var(--border)', marginTop: 8, paddingTop: 8, fontSize: 12 }}><div><strong>{item.request_kind}</strong> · kapsam: {item.scope}</div><div style={{ color: 'var(--text3)', margin: '3px 0 7px' }}>Talep: {new Date(item.created_at).toLocaleString('tr-TR')}</div><button onClick={() => update(item.id, 'verified')} style={{ marginRight: 6 }}>Doğrula</button><button onClick={() => update(item.id, 'rejected')}>Reddet</button></div>) : <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text3)' }}>Bekleyen KVKK talebi yok.</div>}</div>
}
