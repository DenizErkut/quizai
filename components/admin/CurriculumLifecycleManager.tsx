...
export default function CurriculumLifecycleManager() {
  ...
  async function prepareRevision(objective: Objective) {
    ...
    const reason = reasons[objective.id]?.trim() || ''
    if (!versionId || !topicNodeId || reason.length < 3) return setMessage('❌ Taslak sürüm, doğrulanmış konu hedefi ve gerekçe gerekli.')
    setBusy(true); setMessage('')
    try {
  ...
  return <div className="card" style={{ marginTop: '16px' }}>
    ...
    <div style={{ marginTop: 12, display: 'grid', gap: 6 }}>
      {versions.map(version => <div key={version.id} style={{ padding: 8, border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }}>
        <strong>{version.title}</strong> · {version.status === 'active' ? 'aktif' : version.status === 'draft' ? 'taslak' : 'arşiv'}
        {version.status === 'draft' && <button className="btn btn-sm" disabled={busy} onClick={() => activate(version)} style={{ marginLeft: 8 }}>Hazırsa etkinleştir</button>}
      </div>)}
    </div>
    {objectives.length > 0 && <div style={{ marginTop: 16, display: 'grid', gap: 8 }}>
      ...
      {objectives.map(objective => <div key={objective.id} style={{ padding: 10, border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }}>
        ...
        <input value={reasons[objective.id] || ''} onChange={event => setReasons(current => ({ ...current, [objective.id]: event.target.value }))} placeholder="Değişiklik gerekçesi" style={{ marginTop: 7, width: '100%' }} />
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 7 }}>
          ...
          {drafts.length > 0 && <>
            ...
            <select value={revisionTarget[objective.id] || ''} onChange={event => setRevisionTarget(current => ({ ...current, [objective.id]: event.target.value }))}>
              <option value="">— Konu hedefi —</option>{targets.filter(target => target.grade === objective.grade).map(target => <option key={target.topic_node_id} value={target.topic_node_id}>{target.subject} → {target.unit} → {target.topic}</option>)}
            </select>
