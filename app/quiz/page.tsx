...
const TOPIC_MAP: Record<string, { topic: string; subject: string }[]> = {
  ilkokul:    [
    { topic: 'Toplama ve çıkarma', subject: 'Matematik' },
    { topic: 'Hayvanlar alemi', subject: 'Fen' },
    { topic: 'Mevsimler ve iklim', subject: 'Fen' },
    { topic: 'Türkiye haritası', subject: 'Sosyal' },
    { topic: 'Çarpım tablosu', subject: 'Matematik' },
  ],
  ortaokul:   [
    { topic: 'Ondalık sayılar', subject: 'Matematik' },
    { topic: 'Hücre ve organeller', subject: 'Fen' },
    { topic: 'Osmanlı İmparatorluğu kuruluşu', subject: 'Tarih' },
    { topic: 'Denklemler ve eşitsizlikler', subject: 'Matematik' },
    { topic: 'Asal sayılar ve OBEB/OKEK', subject: 'Matematik' },
  ],
  lise:       [
    { topic: 'Türev ve integral temelleri', subject: 'Matematik' },
    { topic: 'Organik kimya', subject: 'Kimya' },
    { topic: 'Atatürk ilkeleri', subject: 'Tarih' },
    { topic: 'Logaritma', subject: 'Matematik' },
    { topic: 'Genetik ve DNA', subject: 'Biyoloji' },
  ],
  üniversite: [
    { topic: 'Uçağın kısımları', subject: 'Havacılık' },
    { topic: 'Termodinamik yasaları', subject: 'Fizik' },
    { topic: 'Diferansiyel denklemler', subject: 'Matematik' },
    { topic: 'Makroekonomi temelleri', subject: 'İktisat' },
    { topic: 'Veri yapıları ve algoritmalar', subject: 'Bilişim' },
  ],
}
...
export default function QuizPage() {
  ...
  async function startQuiz() {
    ...
    const msgs = ['Profilin analiz ediliyor...', 'Müfredat kontrol ediliyor...', 'Sorular oluşturuluyor...', 'Zorluk seviyesi ayarlanıyor...', 'Son kontroller...']
    let mi = 0
  ...
  async function next() {
    if (current + 1 >= questions.length) {
      // Save result
      const score = answers.filter(a => a.correct).length + (chosen === questions[current].ans ? 1 : 0)
      if (sessionId) {
        const { data: { session } } = await supabase.auth.getSession()
          ...
          <div className="anim-up" style={{ marginBottom: '1.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{
                ...
                color: 'var(--accent2)', fontWeight: 600, fontSize: '15px',
              }}>
          ...
          <div className="card anim-up">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {q.opts.map((opt, i) => {
                ...
                let color = 'var(--text)'
                if (chosen !== null) {
                  if (i === q.ans) { bg = 'var(--green-bg)'; border = 'rgba(52,211,153,0.4)'; color = 'var(--green)' }
                  else if (i === chosen) { bg = 'var(--red-bg)'; border = 'rgba(248,113,113,0.4)'; color = 'var(--red)' }
                }
                return (
                  <button key={i} onClick={() => choose(i)} disabled={chosen !== null}
                    style={{
                      ...
                      font: '14px/1.45 "DM Sans", sans-serif', cursor: chosen !== null ? 'default' : 'pointer',
