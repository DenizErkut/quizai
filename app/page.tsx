        ...
        <div style={{ maxWidth: '1060px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px' }}>
            {([
              ...
              { name: 'Google Gemini', sub: 'Semantik Arama', color: '#4285F4', bg: '#f0f4ff', border: 'rgba(66,133,244,0.25)',
                logo: 'https://www.google.com/images/branding/googleg/1x/googleg_standard_color_128dp.png',
                fallbackText: 'Google' },
              { name: 'Supabase', sub: 'Veritabanı & Realtime', color: '#3ECF8E', bg: '#f0fdf9', border: 'rgba(62,207,142,0.25)',
                logo: 'https://supabase.com/favicon/favicon-196x196.png',
                fallbackText: 'Supabase' },
              { name: 'Vercel', sub: 'Hosting & Cron', color: '#000', bg: '#f9fafb', border: 'rgba(0,0,0,0.12)',
                logo: 'https://assets.vercel.com/image/upload/front/favicon/vercel/180x180.png',
                fallbackText: 'Vercel' },
              { name: 'iyzico', sub: 'Güvenli Ödeme', color: '#00B1E1', bg: '#f0fbff', border: 'rgba(0,177,225,0.25)',
                logo: 'https://www.iyzico.com/assets/images/icons/favicon.png',
                fallbackText: 'iyzico' },
              { name: 'Resend', sub: 'E-posta', color: '#000', bg: '#f9fafb', border: 'rgba(0,0,0,0.12)',
                logo: 'https://resend.com/static/favicons/favicon-32x32.png',
                fallbackText: 'Resend' },
              { name: 'Web Push', sub: 'Anlık Bildirimler', color: '#6366f1', bg: '#f5f3ff', border: 'rgba(99,102,241,0.25)',
                logo: null,
                fallbackText: '🔔' },
              { name: 'YouTube', sub: 'Video Önerileri', color: '#FF0000', bg: '#fff5f5', border: 'rgba(255,0,0,0.2)',
                logo: 'https://www.youtube.com/s/desktop/28b0985e/img/favicon_144x144.png',
                fallbackText: 'YouTube' },
              { name: 'Next.js', sub: 'React Framework', color: '#000', bg: '#f9fafb', border: 'rgba(0,0,0,0.12)',
                logo: 'https://nextjs.org/static/favicon/favicon-32x32.png',
                fallbackText: 'Next.js' },
            ] as {name:string;sub:string;color:string;bg:string;border:string;logo:string|null;fallbackText:string}[]).map(p => (
              <div key={p.name} style={{
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                gap: '10px', padding: '18px 12px',
                borderRadius: '16px', background: p.bg,
                border: `1.5px solid ${p.border}`,
                transition: 'all 0.2s', cursor: 'default',
                boxShadow: '0 1px 4px rgba(0,0,0,0.04)',
              }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = `0 8px 24px ${p.border}` }}
                onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 1px 4px rgba(0,0,0,0.04)' }}
              >
                {/* Logo */}
                <div style={{ width: 48, height: 48, borderRadius: '12px', background: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                  {p.logo ? (
                    <img
                      src={p.logo}
                      alt={p.name}
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '10px', color: p.color, fontWeight: 600, marginTop: '2px' }}>{p.sub}</div>
                </div>
              </div>
            ))}
          </div>
