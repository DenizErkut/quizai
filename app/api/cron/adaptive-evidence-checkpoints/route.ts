...
async function measure(evaluation: any, checkpoint: 'day1' | 'day7') {
  const [{ data: mastery }, { data: sessions }] = await Promise.all([
    ...
    db.from('quiz_sessions').select('pct,completed,answers').eq('user_id', evaluation.student_id).gt('created_at', evaluation.observation_started_at).limit(250),
  ])
  ...
  if (checkpoint === 'day7') Object.assign(update, { followup_mastery: values.mastery, followup_retention: values.retention, followup_pct: values.pct, observation_ended_at: values.measured_at })
...
export async function GET(req: NextRequest) {
  ...
  const { data: evaluations, error } = await db.from('adaptive_learning_evaluations').select('id,student_id,observation_started_at,day1_measured_at,day7_measured_at').eq('sample_version', 'adaptive-learning-v3-pilot').is('day7_measured_at', null).order('observation_started_at').limit(250)
