...
export async function POST(req: NextRequest) {
  ...
  const [{ data: mastery }, { data: sessions }, { data: learningProfile }, { data: misconceptions }] = await Promise.all([
    ...
    // misconception_catalog crossing için (bkz. 20260923130000_adaptive_
    // evaluation_misconception_crossing.sql) — aynı dondurma mantığı: pilot
    // ilerledikçe misconception_review müdahaleleri bu kümeyi değiştirebilir.
    db.from('student_misconceptions').select('misconception_id').eq('student_id', body.studentId).eq('status', 'confirmed'),
  ])
  ...
  const baselineMisconceptionIds = [...new Set((misconceptions ?? []).map(row => row.misconception_id))]
  const { data, error } = await db.from('adaptive_learning_evaluations').insert({
    ...
    baseline_misconception_ids: baselineMisconceptionIds,
  }).select('id,student_id,cohort,baseline_mastery,baseline_retention,baseline_pct,baseline_mastery_tier,baseline_learning_pace,baseline_recent_trend,baseline_misconception_ids').single()
