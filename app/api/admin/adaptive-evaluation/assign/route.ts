...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await db.from('adaptive_learning_evaluations').insert({ student_id: body.studentId, cohort: assignedCohort, baseline_mastery: average((mastery ?? []).map(row => Number(row.mastery_score)).filter(Number.isFinite)), baseline_retention: average((mastery ?? []).map(row => Number(row.retention_score)).filter(Number.isFinite)), baseline_pct: average((sessions ?? []).map(row => Number(row.pct)).filter(Number.isFinite)) }).select('id,student_id,cohort,baseline_mastery,baseline_retention,baseline_pct').single()
