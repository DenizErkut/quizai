...
export async function GET(req: NextRequest) {
  ...
  const { data: evaluations, error } = await db.from('adaptive_learning_evaluations').select('id,student_id,cohort,observation_started_at,observation_ended_at,day1_measured_at,day7_measured_at').eq('sample_version', 'adaptive-learning-v3-pilot').order('observation_started_at', { ascending: false }).limit(500)
  ...
  const participants = (evaluations ?? []).map(row => {
    ...
    return { id: row.id, student_id: row.student_id, name: identities[row.student_id]?.full_name || 'İsimsiz', email: identities[row.student_id]?.email || '', cohort: row.cohort, observation_days: observationDays, pilot_tests: tests, completed: Boolean(row.day7_measured_at), day1_done: Boolean(row.day1_measured_at), day7_done: Boolean(row.day7_measured_at), day1_ready: !row.day1_measured_at && observationDays >= 1 && tests >= 1, day7_ready: !row.day7_measured_at && observationDays >= 7 && tests >= 3 }
  })
