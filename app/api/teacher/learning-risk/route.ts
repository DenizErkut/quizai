...
export async function GET(req: NextRequest) {
  ...
  const { data: members, error: memberError } = await db.from('classroom_students').select('classroom_id,student_id').in('classroom_id', targetClasses.map(item => item.id))
  ...
  const { data: mastery, error: masteryError } = await db.from('student_mastery')
    ...
    .in('student_id', studentIds).eq('learning_objective_key', '').gte('attempt_count', 3).gte('confidence_score', 0.3).limit(5000)
  ...
  return NextResponse.json({ classrooms: classrooms ?? [], ...overview, warnings: overview.warnings.slice(0, 100), policy_version: 'predictive-learning-v1', generated_at: new Date().toISOString() })
}
