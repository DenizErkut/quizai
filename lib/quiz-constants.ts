// lib/quiz-constants.ts — Quiz tipleri, sabitler ve yardımcı fonksiyonlar

export type QuestionType = 'multiple_choice' | 'fill_blank' | 'matching' | 'true_false' | 'ordering' | 'short_answer' | 'multi_true_false' | 'table_fill' | 'mixed'

export interface Question {
  q: string; opts: string[]; ans: number; exp: string
  svg?: string | null; qtype?: 'text' | 'svg'; visualQuestionText?: string
  visualContextQuality?: { score: number; reason: string; evaluator: 'openai' }
  bankQuestionId?: string; bankFingerprint?: string
  type?: QuestionType
  blank?: string
...
export interface Profile {
  name: string; grade: string; language: string; plan: string;
  monthly_test_count: number; daily_test_count?: number;
  daily_test_date?: string; onboarding_completed?: boolean
}

export type Screen = 'topic' | 'loading' | 'quiz' | 'result' | 'limit' | 'error'

export function getActiveLang(profileLang?: string): string {
